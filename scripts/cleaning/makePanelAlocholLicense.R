...
business_active = read.csv("data/clean/liquor-licenses/all-licenses-1990-2019-Somdeepa.csv")
...
city_names_crosswalk = read.csv("data/clean/merged/city_name_crosswalk.csv")

policychanges = read.csv("data/clean/merged/treated_cities.csv") %>% 
  left_join(city_names_crosswalk, join_by(city == electiondata))

temp = business_active %>% 
  filter(city %in% policychanges$licensesdata)

businesses_by_year_treatedonly = data.frame(
  year = years,
  total_businesses = unlist(lapply(years, function(x) count_active_businesses(x, temp)))
)
...
## ONLY TREATED CITIES: BY OFF/ON

all_combinations = expand.grid(
  category = unique(temp$category),
  year = seq(startYear, endYear)
)
...
businesses_by_year_category_treatedonly = do.call(rbind, lapply(years, function(x) count_active_businesses_category(x, temp)))

write.csv(businesses_by_year_category_treatedonly, "data/clean/liquor-licenses/total_businesses_by_year_category_treated_only_1990_2019.csv", row.names = F)
...
write.csv(businesses_by_year_treatedonly, "data/clean/liquor-licenses/total_businesses_by_year_treated_only_1990_2019.csv", row.names = F)

## ONLY TREATED: BY CATEGORY DETAIL

all_combinations = expand.grid(
  license_type = unique(temp$category_detail),
  year = seq(startYear, endYear)
)
...
businesses_by_year_category_detail = do.call(rbind, lapply(years, function(x) count_active_businesses_category(x, temp)))

write.csv(businesses_by_year_category_detail, "data/clean/liquor-licenses/total_businesses_by_year_treated_only_category_detail_1990_2019.csv", row.names = F)
